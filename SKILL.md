...
**不要为了「屏幕还很空」就去调大 `.wrap`。**900px 正文栏在 16px 字号下一行约 **54 个汉字** —— 中文正文的舒适区间是 30~45 字/行,57 已经偏长了,**再放宽只会更糟**(1180 → 71 字/行、1400 → 85 字/行)。而且实测同一页从 900 放到 1180,总页高只缩 7%、表格高度**一点没变** —— 说明挤的根本不是正文和表格。

真正装不下的只有**天生就宽**的那几类,给它们加 `.bleed`,让它们单独挣脱正文栏(上限 1400px,自动居中):
