...
> ⚠️ **不要照着旧页面抄 CSS。**旧页面带着它生成那天的字号、宽度、缺失的 `.bleed` —— 抄过来就把过时样式一起继承了,而且**页面看着正常,没人会发现**。实测踩过:模板早已是 16px 正文,新页面照旧页面抄出来还是 14px。
...
**中文比拉丁字母更吃字号**,13~14px 的说明块在 900px 宽的栏里读起来是累的。加大字号还有个附带好处:每行汉字数跟着降(14px 时 62 字/行 → 16px 时 **54 字/行**),更接近舒适区间(见下条)。
...
| **模式 B 改动汇报 / 调查报告**(规则卡 + 表格 + 并排双栏为主) | `.wrap rep` **1280px** | 主体不是散文,900 装不下,而且紧邻的 `.bleed` 会形成断层 |
...
**`.bleed` 上限 1440px**,只给真正天生更宽的内容:并排两个后台外壳、10 列以上宽表、很宽的 ASCII 树。mermaid 宽图**不用手加**,`render-mermaid.py` 按 viewBox > 900px 自动打 `.mmd.wide`。

> ⚠️ **别单独优化每一块,要看它们上下摞在一起是什么样。**
>
> 这是实测翻的车:我按「散文行长」把正文栏定成 900,又按「装得下并排双栏」把 `.bleed` 定成 1400 —— 每一块单独看都对,**摞在同一页上就是断的**。实测一份改动汇报页:规则卡的文字挤在 **701px**,紧贴它下面的并排双栏铺开 **1400px**,差整整一倍,左侧空出 250px 的死白。用户连着三次说「宽度不对」,我前两次都拿行长这条理由挡回去 —— 但**行长规则管的是段落散文**,而改动汇报页的主体是规则卡、表格、代码引用、并排界面,散文只占一小部分。这条理由用错了地方。
>
...
**写完源码卡不算完 —— 必须跑渲染器把它变成真正的图。**只贴源码等于没画:手机上看到的是一坨代码,读者根本不会自己去粘到别处渲染。
...
| 新页面照着旧页面抄 CSS | 把旧页面那天的字号、宽度、缺失的 `.bleed` 一起继承下来 —— **页面看着完全正常,不量根本发现不了**。实测:模板已是 16px 正文,照旧页抄出来的新页仍是 14px | **只从 `_template` 复制起手**;旧页面只抄内容结构,不抄 `<style>` |
...
| 改动汇报靠 `git diff --stat` 摘要写 | 漏掉规则级改动,报告是假的 | 逐行看,逐文件对账 |
