...
复制 `~/www/_template/index.html` 起手,骨架固定这几块:
...
模板的字号阶梯(2026-08-16 整体上调一档,原来偏小):

| 用途 | 字号 |
|---|---|
| 正文 `body` | **15px** |
| 节标题 `.st-h` | 17px |
| 说明块 `.say` / `.hint` / `.lead` / 规则卡正文 | 14px |
| 表格 `td` / `th` | 13px |
| 行内 `code`、小标签 | 12~12.5px |
| 辅助文字(`small`、`.stats .l`) | 10.5~11.5px |

**中文比拉丁字母更吃字号**,13px 的说明块在 900px 宽的栏里读起来是累的。加大字号还有个附带好处:每行汉字数跟着降,更接近舒适区间(见下条)。
...
**不要为了「屏幕还很空」就去调大 `.wrap`。**900px 正文栏在 15px 字号下一行约 **57 个汉字** —— 中文正文的舒适区间是 30~45 字/行,57 已经偏长了,**再放宽只会更糟**(1180 → 74 字/行、1400 → 88 字/行)。而且实测同一页从 900 放到 1180,总页高只缩 7%、表格高度**一点没变** —— 说明挤的根本不是正文和表格。
